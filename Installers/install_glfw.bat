...
set premake_url=https://github.com/glfw/glfw/releases/download/3.3.8/glfw-3.3.8.bin.WIN64.zip
set curr_dir=%~dp0
set dest_dir=%curr_dir%..\Engine\External\glfw

rem --- CREATE PREMAKE DIRECTORY (IF NOT EXISTS) ---
if not exist %dest_dir% mkdir %dest_dir%

rem --- DOWNLOAD AND FORCE EXTRACT PREMAKE EXECUTABLE ---
curl -L %premake_url% > %dest_dir%\glfw.zip
echo Premake ZIP file download completed.
powershell -command "Expand-Archive -Path '%dest_dir%\glfw.zip' -DestinationPath '%dest_dir%' -Force"
